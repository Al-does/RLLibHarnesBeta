...
class WingRewardTask:
    ...
    def __init__(
        ...
        *,
        ...
        self.factor_kernels = controlled_kernels(alpha, x, strength)
        ...
        if size != 1 or self.n_factors == 0:
            raise ValueError("model must be a Cartesian product of Wing factors")
        expected = compose_hmm_factors([wing_model(alpha, x)] * self.n_factors)
        for name in (
            "initial_distribution", "transition_matrix", "emission_matrix",
            ...
            if actual is None or actual.shape != reference.shape or not np.allclose(
                ...
                raise ValueError(f"model {name} does not match independent Wing factors")
        self._model = model
        ...
        if not factors or any(
            ...
            raise ValueError("rewarded_factors must be distinct valid factor indices")
        ...
        self.action_observation_space = gym.spaces.Box(
            low=0.0, high=1.0, shape=(3 * self.n_factors,), dtype=np.float32
        )
    ...
    def _factor_actions(self, action: int) -> tuple[int, ...]:
        if isinstance(action, (bool, np.bool_)) or not self.action_space.contains(action):
            raise ValueError("action is outside the Cartesian Wing action space")
        return np.unravel_index(int(action), (3,) * self.n_factors)

    def resolve_action(
        self, action: int, state: int, model: HMMModel
    ) -> ActionDecision:
